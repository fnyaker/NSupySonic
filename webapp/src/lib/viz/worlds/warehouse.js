...
export default {
  id: "warehouse",
  uses: ["noise3"],
  params: { fog: 1, lamps: 1, strobe: 1, steps: 56, warm: 1 },
...
void main() {
  ...
  float jit = hash12(gl_FragCoord.xy + fract(beats) * 17.0);
  ...
  float strobeK = (uHit.y * amp * P_STROBE * (1.0 - uArc.z) + uHit2.w * 0.6);
  ...
  for (int i = 0; i < 96; i++) {
    ...
    float dens = (0.014 + 0.03 * exp(-(pos.y + 1.0) * 2.0)) * P_FOG;
    ...
    float li = (cone * 4.5 + 0.008) / (0.05 + dot(dl, dl) * 1.1) * lampK;
    ...
    float bk = backK * exp(-(farZ - pos.z) * 0.18) * (0.6 + 0.4 * exp(-abs(pos.y - 0.6)));
    scatter += trans * dens * dt * (sodium * li + strobeC * si + backC * bk);
