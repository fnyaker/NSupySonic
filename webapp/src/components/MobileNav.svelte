<script>
  ...
  $: active = (prefix) =>
    $location === prefix || (prefix !== "/" && $location.startsWith(prefix))
      ? "active"
      : "";
</script>
...
<nav class="mobilenav">
  <a use:link href="/" class={active("/")}>
    <Icon name="home" size={22} /><span class="l">Accueil</span>
  </a>
  ...
  <a use:link href="/podcasts" class={active("/podcast")}>
    <Icon name="mic" size={22} /><span class="l">Podcasts</span>
  </a>
...
<style>
  ...
  @media (max-width: 640px) {
    .mobilenav {
      ...
      left: 0;
      right: 0;
      bottom: 60px; /* above the mini player */
      height: 56px;
      display: flex;
